...
AssignmentResultPayload AssignmentController::createAssignment(const std::string &title, const std::string &description,
    ...
    assignment.setTitle(title).setDescription(description).setCourseId(courseId).setPriority(1); // default priority
    DB &db = DB::getInstance();
    bool result = db.createAssignment(assignment);
    return {result, result ? "" : "Failed to create assignment", assignment};
}
AssignmentResultPayload updateAssignment(int id, const std::string &title, const std::string &description,
    ...
    assignment->setTitle(title).setDescription(description).setPriority(priority).save();
